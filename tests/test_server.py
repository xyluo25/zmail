...
def test_send_and_get(mail_server: MailServer, here):
    ...
    mail_as_dict = {
        'subject': f'新测试邮件test! {datetime_stamp}',
        'content_text': ['测试内容1_1\r\n', '测试内容1_2'],
        'content_html': '<html> \n<body>\n <h1>标题</h1>\n</body>',
        'from': 'ZMAIL测试',
        'to': 'ZMAIL测试',
        'attachments': [
            os.path.join(here, '图标.ico'),
    ...
    while True:
        if now_latest_num < mail_server.stat()[0]:
            ...
            t += 1
            if t == 120:
                raise TimeoutError(f'Test timeout({t * 0.5}s).')
            time.sleep(0.5)
...
def test_send_cc(mail_server: MailServer):
    ...
    assert mail_receive['headers']['Cc'] == f'<{mail_server.username}>'
